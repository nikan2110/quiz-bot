...
def create_answers_and_question(current_user):
    ...
    del current_question["question"] # we don't need the question, because we don't need create button for it. Just need answers
    response = {
...
async def user_exist(callback_query: types.CallbackQuery):
    await bot.send_message(chat_id=callback_query.from_user.id, text=constants.NUMBER_OF_QUESTIONS_TEXT,  reply_markup=inline_keyboard.NUMBER_OF_QUESTIONS)
...
async def difficulty_and_first_question(callback_query: types.CallbackQuery):
    ...
    if len(current_user["questions"]) != 0:
        ...
        await bot.send_message(chat_id=chat_id, text=response['question_text'],  reply_markup=inline_keyboard.create_answers_buttons(response['current_question']))
    else:
